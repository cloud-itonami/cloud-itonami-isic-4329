(ns installation.registry
  ...
  building-installation-project book-of-record draft, the installation-
  domain analog of `demolition.registry`/`finishing.registry`.
  ...
  (`installation.notify`) -- citing the jurisdiction's pre-work hazmat-
  survey legal basis inline so the notice is self-evidencing about which
  ...
  trade equipment or finalizes an installation-completion sign-off (see
  `installation.governor` ns docstring)."
...
(defn register-site-record
  ...
  the RECORD an operator would keep. `installation.governor` independently
...
(defn register-schedule-proposal
  ...
  DRAFT -- a proposed thermal/acoustic-insulation, sound-proofing,
  elevator/escalator-installation or other specialty-installation
  schedule window, NEVER a trade-equipment-dispatch command or an
  ...
  `installation.governor` independently re-verifies the site is
  verified, its pre-work hazmat survey is complete, its jurisdiction's
  fall-protection requirement is met (or the jurisdiction is honestly
...
(defn register-safety-concern-flag
  ...
  insulation-fiber-exposure materials hazard, structural, or fall-
  protection concern for human review. Pure function --
  `:flag-safety-concern` ALWAYS escalates to a human at every phase (see
  `installation.phase`/`installation.governor` ns docstrings), so this
...
(defn register-supply-order-proposal
  ...
  equipment procurement proposal (insulation batts/rolls, acoustic
  panels, elevator/escalator components, specialty installation hardware
  and rental equipment). Pure function -- does not place any real order;
  it builds the RECORD an operator would keep. Proposals above a cost
  threshold, or below the confidence floor, always escalate to a human
...
(defn render-safety-concern-notice
  ...
  sent (mail + phone, `installation.notify`) to the site's supervisor/
  ...
  (let [{:keys [hazmat-survey-basis hazmat-survey-provenance owner-authority]} (facts/spec-basis jurisdiction)]
    (str "# Building-Installation Site Safety-Concern Notice\n\n"
         ...
         "trade-equipment dispatch or an installation-completion sign-off. "
         "Resolution and any installation-completion decision remain the site "
