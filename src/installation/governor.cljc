(ns installation.governor
  ...
  notion of building-installation safety law, whether a site's own
  recorded pre-work hazmat survey / fall-protection measures actually
  satisfy its jurisdiction's requirements, or when a proposal has quietly
  drifted outside this actor's charter into trade-equipment control or
  installation-completion sign-off, so this MUST be a separate system
  able to *reject* a proposal and fall back to HOLD -- the building-
  installation analog of `cloud-itonami-isic-4311`'s Demolition Governor /
  `cloud-itonami-isic-4330`'s Finishing Governor.

  ## Scope -- this is a COORDINATION-ONLY actor

  This actor NEVER controls trade equipment and NEVER finalizes an
  installation-completion sign-off -- that authority is the site
  supervisor / building official's EXCLUSIVELY. Every proposal this
  actor's Installation Advisor can produce carries `:effect :propose` and
  ...
  or piece of equipment was dispatched' or 'an installation-completion
  ...
  CANNOT override them.
    ...
    6. Hazmat survey incomplete  -- for `:schedule-installation-
                                     operation`, has the site's own
                                     recorded `:hazmat-survey-completed?`
                                     ground-truth field (the pre-work
                                     asbestos/hazmat-material survey)
                                     actually been set true?
    7. Fall-protection
       noncompliant               -- for `:schedule-installation-
                                     operation` in a `:quantitative`-
                                     threshold jurisdiction (JPN/USA),
                                     INDEPENDENTLY recompute whether the
                                     site's own recorded `:scaffold-
                                     working-height-m` / `:fall-
                                     protection-installed?` fields meet
                                     its jurisdiction's regulatory fall-
                                     protection trigger (`installation.
                                     facts/fall-protection-noncompliant?`)
  ...
  the human may approve.
    ...
    - `:flag-safety-concern` is UNCONDITIONALLY a member of `high-stakes`
      ...
      confidence or governor cleanliness. Surfacing an insulation-fiber-
      exposure, structural or fall-protection concern is exactly the kind
    ...
    - `:schedule-installation-operation` is DELIBERATELY **NOT** a
      permanent member of `high-stakes` -- this is the same structural
      difference `finishing.governor`'s own schedule op establishes from
      `demolition.governor`/`roadrail.governor`'s schedule ops (which ARE
      ...
      / public-traffic / buried-utility risk). Thermal/acoustic
      insulation, sound-proofing and other specialty building-
      installation scheduling is a materially LOWER-risk coordination
      artifact -- once the eight HARD checks above are clean (site
      verified, hazmat survey complete, no fall-protection violation, no
      unresolved concern, legal basis cited) AND confidence clears
      `confidence-floor`, it is treated the SAME as `:order-supplies`: it
      MAY auto-commit at phase 3 (see `installation.phase` ns docstring).
      It still escalates like any other write whenever confidence is low
      or the rollout phase does not yet allow auto-commit. The HARD
      checks (1-8) always apply regardless of phase or confidence -- only
...
(def closed-op-allowlist
  #{:log-site-record :schedule-installation-operation :flag-safety-concern :order-supplies})
...
(def high-stakes
  ...
  `:schedule-installation-operation` is ALSO deliberately NOT a member --
  ...
  (let [v (:value proposal)]
    (when (and (map? v)
               (or (true? (:trade-equipment-control? v))
                   ...
                   (true? (:finalizes-installation-completion-sign-off? v))))
      [{:rule :forbidden-action-class
        :detail "工事用機材の直接操作コマンド、または設置完了サインオフの確定を伴う提案は恒久的に禁止（現場監督/建築主事の専権事項）"}])))
  ...
  (when (= op :schedule-installation-operation)
    (let [value (:value proposal)]
      (when (or (empty? (:cites proposal))
        ...
        [{:rule :no-legal-basis
          :detail "公式legal-basisの引用が無い提案は設置工事スケジュール提案として扱えない"}]))))
    ...
    (let [a (store/site st subject)]
      (when-not (true? (:hazmat-survey-completed? a))
        [{:rule :hazmat-survey-incomplete
          :detail (str subject " は事前調査（石綿等のhazmat survey）が完了記録されていない状態での設置工事スケジュール提案")}]))))

(defn- fall-protection-noncompliant-violations
  "For `:schedule-installation-operation`, INDEPENDENTLY recompute
  whether the site's own recorded scaffold-working-height/fall-
  protection fields meet its jurisdiction's regulatory fall-protection
  trigger via `installation.facts/fall-protection-noncompliant?`. Fires
  ONLY when that returns `true` (a :quantitative jurisdiction confirmed
    ...
    (let [a (store/site st subject)]
      (when (true? (:safety-concern-unresolved? a))
        [{:rule :unresolved-safety-concern
          :detail (str subject " は未解決の安全性懸念（断熱繊維暴露/構造/墜落防止）がある状態での設置工事スケジュール提案")}]))))
