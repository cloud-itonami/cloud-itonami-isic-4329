(ns installation.operation
  "OperationActor -- one building-installation-project coordination
  operation = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (Installation Advisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the Installation
  ...
  One graph run = one building-installation-coordination operation
  ...
  `:flag-safety-concern` ALWAYS reaches this node -- see `installation.
  phase`/`installation.governor` ns docstrings. `:log-site-record`,
  `:schedule-installation-operation` and `:order-supplies` (below the
  cost threshold) MAY auto-commit at phase 3 when the governor is clean
  -- see `installation.phase` ns docstring.
  ...
  `installation.notify`) to the site's `:safety-contacts` roster --
