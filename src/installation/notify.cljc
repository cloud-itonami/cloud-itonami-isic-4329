(ns installation.notify
  ...
  other-construction-installation-domain analog of `demolition.notify`/
  `finishing.notify`.
  ...
  human first (see `installation.phase`/`installation.governor` ns
  docstrings), so by the time `installation.operation`'s `:commit` node
  ...
  `installation.notify` itself runnable unmodified on JVM Clojure,
