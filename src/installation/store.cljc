(ns installation.store
  "SSoT for the building-installation-coordination actor, behind a `Store`
  ...
  counter: `:log-site-record` (site-record-log), `:schedule-installation-
  ...
  authorizes trade-equipment use, or signs off on installation completion
  -- it only ever proposes, logs and schedules (`:effect :propose`
  unconditionally, see `installation.governor` ns docstring). The site's
  own `:site-verified?` / `:hazmat-survey-completed?` / `:safety-concern-
  unresolved?` / `:scaffold-working-height-m` / `:fall-protection-
  installed?` ground-truth fields are what the Installation Governor
  independently re-checks before a `:schedule-installation-operation`
  proposal may ever commit (see `installation.governor`).
...
(defprotocol Store
  ...
  (all-sites [s])
  (ledger [s])
  (site-record-log-history [s] "the append-only site-record-log history (installation.registry drafts)")
...
(defn demo-data
  ...
  interior attic-insulation retrofit -- no scaffold height involved), the
  uncovered-jurisdiction / not-verified / hazmat-survey-incomplete /
  fall-protection-noncompliant / unresolved-safety-concern failure modes
  ...
  []
  {:sites
   {"site-1" {:id "site-1" :name "Sakura Heights Apartments -- Attic Insulation Retrofit"
              :jurisdiction "JPN" :trades [:insulation]
    ...
    "site-2" {:id "site-2" :name "Atlantis Waterfront Tower -- Elevator Modernization"
              :jurisdiction "ATL" :trades [:elevator-installation]
              :site-verified? true :hazmat-survey-completed? true
              :hazmat-detected? false :safety-concern-unresolved? false
              :scaffold-working-height-m 3.0 :fall-protection-installed? true
              :safety-contacts []
              :status :intake}
    "site-3" {:id "site-3" :name "鈴木ビル改修 -- 遮音間仕切り設置工事"
              :jurisdiction "JPN" :trades [:sound-proofing]
              ...
              :status :unverified}
    "site-4" {:id "site-4" :name "田中団地 -- 断熱改修工事"
              :jurisdiction "JPN" :trades [:insulation]
    ...
    "site-5" {:id "site-5" :name "ことぶき小学校 -- 屋根断熱・エスカレーター設置工事"
              :jurisdiction "JPN" :trades [:insulation :escalator-installation]
              :site-verified? true :hazmat-survey-completed? true
              :hazmat-detected? false :safety-concern-unresolved? false
    ...
    "site-6" {:id "site-6" :name "山田マンション -- エレベーター新設工事"
              :jurisdiction "JPN" :trades [:elevator-installation]
              :site-verified? true :hazmat-survey-completed? true
              ...
              :status :concern-open}
    "site-7" {:id "site-7" :name "Riverside Plaza -- Roof Insulation & Elevator Upgrade"
              :jurisdiction "USA" :trades [:insulation :elevator-installation]
              :site-verified? true :hazmat-survey-completed? true
              :hazmat-detected? false :safety-concern-unresolved? false
              ...
              :status :ready-to-schedule}
    "site-8" {:id "site-8" :name "Alt-Fabrikgelände -- Schallschutz-Sanierung"
              :jurisdiction "DEU" :trades [:sound-proofing]
              :site-verified? true :hazmat-survey-completed? true
              :hazmat-detected? false :safety-concern-unresolved? false
