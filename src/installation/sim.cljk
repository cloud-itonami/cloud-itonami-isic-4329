(ns installation.sim
  "Demo driver -- `clojure -M:dev:run`. Walks building-installation sites
  through a full coordination episode, exercising every governor check:
  log a site record (auto-commits) -> propose an installation-operation
  schedule for a clean, verified site (AUTO-COMMITS at phase 3, UNLIKE
  the sibling demolition/road-rail actors -- see `installation.governor`/
  `installation.phase` ns docstrings) -> flag a safety concern (ALWAYS
...
(defn -main [& _]
  (let [db (store/seed-db)
    ...
    (println "== schedule-installation-operation site-1 (clean, interior work, high confidence -- AUTO-COMMITS at phase 3) ==")
    (println (exec! actor "t2" {:op :schedule-installation-operation :subject "site-1"
                                :trade :insulation
                                :window {:proposed-start-date "2026-08-01" :proposed-end-date "2026-08-10"}
                                :notes "屋根裏断熱材吹込み工事"} operator))
    ...
    (let [r (exec! actor "t3" {:op :flag-safety-concern :subject "site-1"
                               ...
                               :concern-description "既存断熱材の撤去中にグラスウール粉じんの飛散を確認、追加調査が必要。"} operator)]
      (println r)
    ...
    (println (exec! actor "t7" {:op :schedule-installation-operation :subject "site-2" :trade :elevator-installation :window {}} operator))

    (println "== schedule-installation-operation site-3 (site-verified? false -> HARD hold, never reaches a human) ==")
    (println (exec! actor "t8" {:op :schedule-installation-operation :subject "site-3" :trade :sound-proofing :window {}} operator))

    (println "== schedule-installation-operation site-4 (hazmat-survey-completed? false -> HARD hold) ==")
    (println (exec! actor "t9" {:op :schedule-installation-operation :subject "site-4" :trade :insulation :window {}} operator))

    (println "== schedule-installation-operation site-5 (JPN, scaffold-working-height-m=3.5 >= 2m trigger, fall-protection-installed?=false -> HARD hold) ==")
    (println (exec! actor "t10" {:op :schedule-installation-operation :subject "site-5" :trade :escalator-installation :window {}} operator))

    (println "== schedule-installation-operation site-6 (safety-concern-unresolved? true on file -> HARD hold) ==")
    (println (exec! actor "t11" {:op :schedule-installation-operation :subject "site-6" :trade :elevator-installation :window {}} operator))
    ...
    (println "== USA (quantitative, 1.8m/6ft trigger) -- schedule-installation-operation site-7, height=2.4m, fall-protection installed -- compliant (AUTO-COMMITS) ==")
    (println (exec! actor "t13" {:op :schedule-installation-operation :subject "site-7" :trade :insulation
                                 :window {:proposed-start-date "2026-09-01" :proposed-end-date "2026-09-10"}} operator))

    (println "== DEU/EU (qualitative -- no fixed numeric trigger, never fabricated) -- schedule-installation-operation site-8 (AUTO-COMMITS, confidence clean) ==")
    (println (exec! actor "t14" {:op :schedule-installation-operation :subject "site-8" :trade :sound-proofing
