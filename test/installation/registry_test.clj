...
(def sample-site
  {:id "site-1" :name "Sakura Heights Apartments -- Attic Insulation Retrofit" :jurisdiction "JPN"})

(deftest safety-concern-notice-cites-legal-basis-inline
  (let [doc (r/render-safety-concern-notice sample-site "JPN-SCF-000000" "既存断熱材にグラスウール粉じん飛散の疑いあり")]
    (is (re-find #"JPN-SCF-000000" doc))
    (is (re-find #"石綿障害予防規則" doc))
    (is (re-find #"laws\.e-gov\.go\.jp" doc))
    (is (re-find #"グラスウール粉じん" doc))
    (is (re-find #"site supervisor" doc) "honestly disclaims installation-completion-sign-off authority")))
