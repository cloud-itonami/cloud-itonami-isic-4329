...
(deftest read-parity
  (doseq [[label s] (backends)]
    (testing label
      (is (= "Sakura Heights Apartments -- Attic Insulation Retrofit" (:name (store/site s "site-1"))))
  ...
  (doseq [[label s] (backends)]
    (testing label
      (testing "log-site-record commits a patch and appends a site-record-log entry"
        ...
        (is (= "Sakura Heights Apartments -- Attic Insulation Retrofit" (:name (store/site s "site-1"))) "unrelated field preserved")
      ...
      (testing "order-supplies drafts a record and advances the sequence"
        (store/commit-record! s {:op :order-supplies :path ["site-1"]
                                 :value {:site-id "site-1" :items ["glass-wool-batt"] :cost-usd 500}})
