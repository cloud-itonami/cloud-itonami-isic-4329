(ns installation.phase-test
  ...
  phase's `:auto` set. `:log-site-record`, `:schedule-installation-
  operation` and `:order-supplies` ARE auto-eligible at phase 3 -- see
  `installation.phase` ns docstring 'Actuation' section (this repo
  deliberately differs from `demolition.phase`/`roadrail.phase`, which
  never auto-commit their own schedule op; it mirrors `finishing.phase`)."
...
(deftest log-site-record-schedule-and-order-supplies-are-auto-eligible-at-phase-3
  ...
  (is (contains? (:auto (get phase/phases 3)) :schedule-installation-operation))
  (is (contains? (:auto (get phase/phases 3)) :order-supplies)))

(deftest write-ops-is-exactly-the-closed-four-op-allowlist
  (is (= #{:log-site-record :schedule-installation-operation :flag-safety-concern :order-supplies}
         phase/write-ops)))
...
(deftest phase-3-auto-set-is-exactly-log-site-record-schedule-and-order-supplies
  (is (= #{:log-site-record :schedule-installation-operation :order-supplies} (:auto (get phase/phases 3)))))
...
(deftest gate-auto-commits-schedule-installation-operation-when-clean-at-phase-3
...
(deftest gate-escalates-schedule-installation-operation-not-yet-auto-eligible-in-earlier-phases
  (testing "phase 2 allows the write but not the auto-commit"
    (is (= :hold (:disposition (phase/gate 2 {:op :schedule-installation-operation} :commit)))
        "phase 2's :writes doesn't include :schedule-installation-operation at all -> phase-disabled hold")))
